...
```bash
cat _first_half_games.txt | xargs python3 classify_art.py --no-masked 2>&1
```
...
- Do NOT modify `classify_art.py`, `corrections.json`, or `_parallel_games.txt`
...
- `classify_art.py` — no code changes
