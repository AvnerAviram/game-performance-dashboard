...
export function parseTableRows(htmlOrElement) {
  const element = typeof htmlOrElement === 'string' 
    ? createElementFromHTML(htmlOrElement)
    : htmlOrElement;
...
export function parseComparisonCards(htmlOrElement) {
  ...
  return cards.map(card => {
    ...
    const subtitle = card.querySelector('.text-xs.text-amber-600, .text-xs.text-purple-600, .text-xs.text-emerald-600, .text-xs.text-red-600, .text-xs.text-cyan-600, .text-xs.text-indigo-600')?.textContent?.trim();
