...
export async function loadTestData() {
  ...
  const avgTheoThemes = themesRaw.length > 0
    ? themesRaw.reduce((s, t) => s + t.avg_theo_win, 0) / themesRaw.length
    : 1;
  const avgTheoMechanics = mechanicsRaw.length > 0
    ? mechanicsRaw.reduce((s, m) => s + m.avg_theo_win, 0) / mechanicsRaw.length
    : 1;
