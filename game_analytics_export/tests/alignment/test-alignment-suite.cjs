 ...
 * Run: node tests/alignment/test-alignment-suite.cjs
 */
...
async function runAlignmentTests() {
    ...
    for (const pageInfo of allPages) {
        ...
        // Take screenshot
        const screenshotPath = path.join(testResultsDir, `${pageInfo.id}-header.png`);
    ...
    // Save results
    fs.writeFileSync(
        path.join(testResultsDir, 'alignment-test-results.json'),
