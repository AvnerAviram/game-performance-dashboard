...
describe('AUTO-RUN: Sorting & Filter Validation', () => {
    ...
    describe('✅ FILTER LOGIC VERIFICATION', () => {
        it('should filter Market Leaders (high game count)', () => {
            ...
            const threshold = sorted[Math.floor(sorted.length * 0.2)]['Game Count'];
            ...
            leadersSorted.forEach((t, i) => {
                console.log(
                    `    ${i + 1}. ${t.Theme}: Perf=${t['Avg Theo Win Index'].toFixed(2)}, Games=${t['Game Count']}`
                );
            });

            // Should be sorted by Performance Index
            for (let i = 0; i < leadersSorted.length - 1; i++) {
                expect(leadersSorted[i]['Avg Theo Win Index']).toBeGreaterThanOrEqual(
                    leadersSorted[i + 1]['Avg Theo Win Index']
                );
            }
        ...
        it('should filter Opportunities (5+ games, <5% market)', () => {
            ...
            const opportunities = mockThemes.filter(
                t => t['Game Count'] >= 5 && t['Avg Theo Win Index'] >= avgPerf && t['Market Share %'] < 5
            );
            ...
            oppSorted.forEach((t, i) => {
                console.log(
                    `    ${i + 1}. ${t.Theme}: Perf=${t['Avg Theo Win Index'].toFixed(2)}, Games=${t['Game Count']}, Market=${t['Market Share %']}%`
                );
            });
        ...
        it('should filter Premium Quality (Top 25% by performance)', () => {
            ...
            for (let i = 0; i < premiumSorted.length - 1; i++) {
                expect(premiumSorted[i]['Avg Theo Win Index']).toBeGreaterThanOrEqual(
                    premiumSorted[i + 1]['Avg Theo Win Index']
                );
            }
    ...
    describe('✅ EXPECTED BEHAVIOR', () => {
        it('should confirm Fairy Tale appears at top of multiple filters', () => {
            ...
            const threshold = sortedByCount[Math.floor(sortedByCount.length * 0.2)]['Game Count'];
            const leaders = mockThemes
                .filter(t => t['Game Count'] >= threshold)
            ...
            const premium = mockThemes
                .filter(t => t['Avg Theo Win Index'] >= perfThreshold)
                .sort((a, b) => b['Avg Theo Win Index'] - a['Avg Theo Win Index']);
