...
describe('Providers and Games Pages - Unit Tests', () => {
    ...
    describe('Providers Aggregation Logic', () => {
        ...
        it('should calculate correct averages', () => {
            ...
            games.forEach(game => {
                ...
                if (game.rtp) {
                    const rtpVal = typeof game.rtp === 'string' ? parseFloat(game.rtp) : game.rtp;
                    if (!isNaN(rtpVal) && rtpVal > 0) {
                        providers[studio].rtp_sum += rtpVal;
                        providers[studio].rtp_count++;
                    }
                }
            });
            ...
            Object.entries(providers).forEach(([name, p]) => {
                ...
                console.log(
                    `✅ ${name}: avg_theo=${avg_theo.toFixed(2)}, avg_rtp=${avg_rtp ? avg_rtp.toFixed(1) : 'N/A'}`
                );
            });
        });
    ...
    describe('Games List Logic', () => {
        ...
        it('should sort by theo_win correctly', () => {
            ...
            console.log(
                ...
                sorted
                    .slice(0, 3)
                    .map(g => `${g.name} (${g.theo_win?.toFixed(2) || 'N/A'})`)
                    .join(', ')
            );
        });
        ...
        it('should search by name correctly', () => {
            ...
            const filtered = games.filter(g => g.name.toLowerCase().includes(testSearch));
        ...
        it('should handle combined filters correctly', () => {
            ...
            console.log(
                `✅ Combined filter (provider="${testProvider}" AND search="${testSearch}"): ${filtered.length} games`
            );
        });
    });
    ...
    describe('Data Completeness', () => {
        it('should have high coverage for critical fields', () => {
            ...
            console.log(
                `   Provider: ${stats.has_provider}/${total} (${((stats.has_provider / total) * 100).toFixed(1)}%)`
            );
            console.log(`   Theme: ${stats.has_theme}/${total} (${((stats.has_theme / total) * 100).toFixed(1)}%)`);
            console.log(
                `   Mechanic: ${stats.has_mechanic}/${total} (${((stats.has_mechanic / total) * 100).toFixed(1)}%)`
            );
            console.log(
                `   Performance: ${stats.has_performance}/${total} (${((stats.has_performance / total) * 100).toFixed(1)}%)`
            );
            console.log(`   RTP: ${stats.has_rtp}/${total} (${((stats.has_rtp / total) * 100).toFixed(1)}%)`);
            console.log(
                `   Volatility: ${stats.has_volatility}/${total} (${((stats.has_volatility / total) * 100).toFixed(1)}%)`
            );
        });
    });
});
