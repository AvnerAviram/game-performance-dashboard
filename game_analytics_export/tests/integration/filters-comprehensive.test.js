...
describe('Mechanic Filters', () => {
    ...
    test('filter by rare mechanic reduces results significantly', () => {
        ...
        const sortedMechanics = [...gameData.mechanics].sort((a, b) => a['Game Count'] - b['Game Count']);
...
describe('Sorting', () => {
    ...
    test('can sort themes by game count', () => {
        const sorted = [...gameData.themes].sort((a, b) => b['Game Count'] - a['Game Count']);

        for (let i = 0; i < sorted.length - 1; i++) {
            expect(sorted[i]['Game Count']).toBeGreaterThanOrEqual(sorted[i + 1]['Game Count']);
        }
    });

    test('can sort themes by avg theo win', () => {
        const sorted = [...gameData.themes].sort((a, b) => b['Avg Theo Win Index'] - a['Avg Theo Win Index']);
    ...
    test('can sort themes by market share', () => {
        const sorted = [...gameData.themes].sort((a, b) => b['Market Share %'] - a['Market Share %']);

        for (let i = 0; i < sorted.length - 1; i++) {
            expect(sorted[i]['Market Share %']).toBeGreaterThanOrEqual(sorted[i + 1]['Market Share %']);
        }
    });

    test('can sort themes alphabetically', () => {
        const sorted = [...gameData.themes].sort((a, b) => a.Theme.localeCompare(b.Theme));
