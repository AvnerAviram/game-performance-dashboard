...
import { initializeDatabase, getOverviewStats, getThemeDistribution, getMechanicDistribution, getProviderDistribution, getAnomalies, getAllGames } from '../../src/lib/db/duckdb-client.js';
import { 
...
describe('DuckDB Aggregations: Overview Stats', () => {
  ...
  test('DuckDB avg_theo_win should match manual calculation', () => {
    const match = compareWithTolerance(
      duckdbStats.avg_theo_win,
      manualStats.avg_theo_win,
      0.01
    );
...
describe('DuckDB Aggregations: Theme Distribution', () => {
  ...
  test('DuckDB avg_theo_win should match manual calculations', () => {
    ...
    manualThemes.forEach(manualTheme => {
      ...
      const match = compareWithTolerance(
        duckdbTheme.avg_theo_win,
        manualTheme.avg_theo_win,
        0.01
      );
  ...
  test('DuckDB total_market_share should match manual calculations', () => {
    ...
    manualThemes.forEach(manualTheme => {
      ...
      const match = compareWithTolerance(
        duckdbTheme.total_market_share,
        manualTheme.total_market_share,
        0.01
      );
...
describe('DuckDB Aggregations: Mechanic Distribution', () => {
  ...
  test('DuckDB avg_theo_win should match manual calculations', () => {
    ...
    manualMechanics.forEach(manualMech => {
      ...
      const match = compareWithTolerance(
        duckdbMech.avg_theo_win,
        manualMech.avg_theo_win,
        0.01
      );
...
describe('DuckDB Aggregations: Provider Distribution', () => {
  ...
  test('DuckDB game counts should match manual counts', () => {
    ...
    manualProviders.forEach(manualProv => {
      const duckdbProv = duckdbProviders.find(p => 
        p.provider_studio === manualProv.provider_studio &&
        p.provider_parent === manualProv.provider_parent
      );
...
describe('DuckDB Aggregations: Anomalies', () => {
  ...
  test('all high performers should have anomaly flag', () => {
    const missingFlag = duckdbAnomalies.high.filter(g => 
      g.performance_anomaly !== 'high'
    );
