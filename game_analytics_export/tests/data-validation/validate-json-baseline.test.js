...
describe('JSON Baseline: Value Ranges', () => {
  ...
  test('volatility should be valid value', () => {
    const validVolatilities = ['low', 'medium', 'high', 'very high', 'very low', 'medium-high', 'medium-low', 'low-medium'];
    ...
    if (invalidVolatility.length > 0) {
      console.error('Games with invalid volatility:', invalidVolatility.map(g => ({ name: g.name, vol: g.volatility ?? g.specs?.volatility })).slice(0, 5));
    }
...
describe('JSON Baseline: Distributions', () => {
  ...
  test('top themes should have reasonable game counts', () => {
    ...
    expect(topTheme.game_count).toBeLessThan(200);
