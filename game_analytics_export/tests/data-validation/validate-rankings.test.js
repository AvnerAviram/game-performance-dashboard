...
describe('Ranking Calculations - Themes', () => {
    ...
    test('all themes should have positive game counts', () => {
        const invalidCounts = gameData.themes.filter(t => !t['Game Count'] || t['Game Count'] <= 0);
    ...
    test('top 10 themes should have reasonable values', () => {
        ...
        top10.forEach((theme, i) => {
            console.log(
                `${i + 1}. ${theme.Theme}: ${theme['Game Count']} games, Performance Index: ${theme['Smart Index'].toFixed(4)}`
            );
...
describe('Ranking Calculations - Mechanics', () => {
    ...
    test('top mechanic should have most games', () => {
        const mechanics = [...gameData.mechanics].sort((a, b) => b['Game Count'] - a['Game Count']);
    ...
    test('all mechanics should have positive game counts', () => {
        const invalidCounts = gameData.mechanics.filter(m => !m['Game Count'] || m['Game Count'] <= 0);
