...
const dataPath = path.join(__dirname, 'game_data_master.json');
...
// ── Generic single-word franchise denylist ───────────────────────────────
// These are common English words that appear as shared prefixes but are NOT real brands.
// Single-word licensed IPs (Buffalo, Monopoly, Slingo, etc.) bypass this via Step 1.
const GENERIC_WORD_DENYLIST = new Set([
  'adventures', 'age', 'book', 'burn', 'coins', 'defenders', 'dragons',
  'empire', 'fiesta', 'fortunes', 'gates', 'gems', 'get', 'goddess', 'gods',
  'gold', 'guardians', 'house', 'jack', 'king', 'knights', 'lady', 'lamp',
  'legacy', 'legend', 'legends', 'lock', 'luck', 'magic', 'pirates',
  'power', 'queen', 'reign', 'return', 'riches', 'rise', 'rome', 'sands',
  'scroll', 'secrets', 'sevens', 'shields', 'spirit', 'story', 'tales',
  'temple', 'ticket', 'tiger', 'tomb', 'tome', 'treasures', 'twice',
  'valley', 'ways', 'wings', 'wonders',
]);
...
for (const [prefix, idxSet] of Object.entries(families)) {
  ...
  if (franchise.split(/\s+/).length === 1 && GENERIC_WORD_DENYLIST.has(franchise.toLowerCase())) {
    genericSkipped++;
    continue;
  }
...
console.log(`  Generic single-word brands filtered: ${genericSkipped}`);
console.log(`\nTop franchises:`);
