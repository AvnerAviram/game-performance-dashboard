...
**Pre-flight gate added**: `classify_art.py` now blocks batches where <80% of games have screenshots. To override: `--allow-text-only` (use with caution, results will be lower quality).
...
python3 classify_art.py --select-batch 30

# Classify specific games
python3 classify_art.py game-A.html game-B.html --output /tmp/batch.json

# Run regression against ground truth
python3 classify_art.py --regression

# Classify without screenshot (text-only)
python3 classify_art.py game-A.html --no-vision

# Classify and compare against GT inline
python3 classify_art.py game-A.html --gt-compare
```
...
├── classify_art.py              ← Main pipeline script (1700+ lines)
