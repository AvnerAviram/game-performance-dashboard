...
if (process.argv[1]?.includes('slotcatalog-scraper')) {
  const missingGamesPath = path.join(__dirname, '../../../../data/staging/missing_games_to_recover.json');
  ...
  // Save results
  const outputPath = path.join(__dirname, '../state/slotcatalog_results.json');
