...
- Color: area-threshold (>=25% of screen) based, 1-3 colors
- art_style dropped
...
SC_DIR = str(SC_CACHE_DIR)
SCREENSHOT_DIR = str(SCREENSHOTS_DIR)
MASTER_PATH = str(MASTER_JSON)
ENV_PATH = str(ENV_FILE)
PIPELINE_DIR = str(ART_PIPELINE_DIR)
...
OUTPUT_PATH = os.path.join(PIPELINE_DIR, 'art_v2_results.json')
PRESCREEN_RESULTS_PATH = os.path.join(os.path.dirname(PIPELINE_DIR), 'prescreen_pipeline', 'state', 'prescreen_results.json')
...
COLOR_VOCABULARY = [
    ...
    "Gray", "Light Blue", "Dark Blue", "Beige", "Dark Green", "Light Green",
]
...
VALID_THEMES = [
    ...
    "Jungle/Rainforest", "Deep Ocean/Underwater", "Atlantis/Lost City",
    ...
    "American Patriotic", "Love Story/Romance", "Quest/Adventure/Journey",
...
VALID_ELEMENTS_EFFECTS = [
    ...
    "Magic Energy/Spell Effects", "Fireworks", "Crystals/Crystal Formations",
]
...
VALID_ELEMENTS_DECOR = [
    ...
    "Cash/Dollar Bills/Money", "Surfboard",
]
...
COLOR CLASSIFICATION RULES:
  For each dominant color visible on screen, estimate what PERCENTAGE of the total visible game area
  it occupies (background + frame + reel area combined). Only report colors covering approximately
  25% or more of the screen. You may report 1, 2, or 3 colors — never more than 3.
  Order by dominance (largest area first).
  ...
  NOT: The color of individual reel symbols. The color described in the game name (unless it matches what you see).
       The operator/casino website header bar at the very top of screenshots.

  DARK BACKGROUNDS DOMINATE — DO NOT IGNORE THEM:
  - Most slot games have large dark backgrounds (Black, Gray, Dark Blue, Dark Green).
  - A Black/Gray/Dark Blue background covering 50-70% of screen IS the dominant color — report it FIRST.
  - Gold trim, glowing accents, or bright decorations on top of a dark BG are usually <25% — do NOT over-report them.
  - Ask: "If I squint, what color covers the MOST pixels?" — it's almost always the background.
  - Common mistake: calling "Gold" dominant when there's a large black/gray background with small gold accents.

  SATURATED ACCENTS ARE SMALLER THAN THEY LOOK:
  - Purple, Neon Blue, Teal, Gold glow/trim LOOK dominant because they're vivid — but measure their PIXEL AREA.
  - A glowing purple border or neon outline covers much less area than it appears. Usually 10-15%, not 25%+.
  - BEFORE reporting a saturated/vivid accent at ≥25%, mentally mask it: does the remaining image lose one-quarter of its area? If not, it's <25%.
  - If the game has a large muted/dark background + scattered bright accents, the accents are almost certainly <25% EACH.

  AREA ESTIMATION GUIDE:
  - Consider the FULL visible game area: background sky/scenery, reel area tinting, frame/border, side panels
  - A color covering ~25% = roughly one-quarter of what you see — significant and clearly noticeable
  - A color at ~15% or less = trim, accents, sparkles — do NOT include in art_color_tone
  - When unsure if a color hits 25%, err on the side of excluding it (accents are noise)

  COLOR SHADE GUIDE (use specific shade when clearly distinct):
  - Dark backgrounds with no stars/sky → "Black". Night sky → "Dark Blue".
  - Ocean/deep water → "Dark Blue". Bright sky/ice → "Light Blue".
  - Jungle/forest → "Dark Green". Meadow/lime → "Light Green" or "Green".
  - Bright pink (bubblegum, candy) → "Pink". Darker rose/magenta shades → "Red" or "Crimson".
  - If you can't tell if it's dark or light, use the base color (Blue, Green, etc.).
  ...
  - A game 85% dark blue with thin gold trim → only ["Dark Blue"] (gold is <25%)
  - A game 50% black + 30% gold → ["Black", "Gold"]
  - A game 45% purple + 30% gold + 25% black → ["Purple", "Gold", "Black"]
  - A game 60% black + purple glow borders + neon accents → ["Black"] only (purple/neon are <25% each)
  - A game 50% green forest + 25% blue sky + scattered gold sparkles → ["Green", "Blue"] (gold <25%)
  - Fire/flame games: what color are the flames? Blue fire → "Blue". Orange fire → "Orange".
  - If screenshot is available, use what you ACTUALLY SEE, not what you'd guess from the theme.
  - A game HEAVILY dominated by one color (>70%) may have only 1 color listed — that is correct.
"""
...
ELEMENT CLASSIFICATION RULES:
  ...
  - "Magic Energy/Spell Effects" — ONLY for actual visible magical bolts, spell circles, energy orbs, or rune glow. Ambient blue/purple lighting, underwater shimmer, or atmospheric color tints are NOT magic energy. If the "magic" is just the general color of the scene's lighting, do NOT list it.
  - "Lightning/Electricity" — ONLY for actual lightning bolts, electrical arcs, or crackling energy visibly rendered. Decorative metalwork patterns (Norse knotwork, ornate borders), glowing outlines, or gradient backgrounds are NOT lightning. If you can't trace a distinct bolt or arc shape, do NOT list it.
"""
...
def build_system_prompt(training_ref):
    return f"""You are an expert slot game visual art classifier with perfect vision. You have exceptional attention to detail and never miss small objects in the background, side panels, or periphery of images. You analyze game screenshots and/or human-written reviews to classify a game's visual art across multiple dimensions.
...
COLOR (only colors covering >=25% of screen area, max 3): {json.dumps(COLOR_VOCABULARY)}
...
{{
  ...
  "art_color_tone_raw": [{{"color": "Gold", "pct": 55}}, {{"color": "Red", "pct": 30}}, {{"color": "Blue", "pct": 10}}],
  "art_color_tone": ["Gold", "Red"],
...
IMPORTANT for art_color_tone and art_color_tone_raw:
- In art_color_tone_raw, list ALL colors you notice with their estimated screen percentage (even below 25%).
  Order by dominance. This is for calibration — include up to 5 colors with percentages.
- In art_color_tone, list ONLY the colors from art_color_tone_raw that cover >=25% of visible screen area.
  This means 1, 2, or 3 colors. A monochrome game gets 1 color. That is correct.
- Percentages are approximate — they should roughly sum to 80-100% (some area may be mixed/transitional).
- Use the FULL screenshot (background, frame, reel area) to estimate coverage.
  IGNORE the operator header bar at the very top edge."""


def extract_review(fname):
    path = os.path.join(SC_DIR, fname)
    if not os.path.exists(path):
        name = fname.replace('.html', '').replace('-', ' ')
        return name, ''
...
COLOR_ALIASES = {
    ...
    "ice blue": "Light Blue", "baby blue": "Light Blue",
    "navy": "Dark Blue", "cobalt": "Dark Blue", "midnight blue": "Dark Blue",
    "deep blue": "Dark Blue", "royal blue": "Dark Blue",
    ...
    "olive": "Dark Green",
    "lime": "Light Green", "mint": "Light Green", "light green": "Light Green",
    "beige": "Beige",
...
ELEMENT_ALIASES = {
    ...
    "crystals": "Crystals/Crystal Formations", "crystal formations": "Crystals/Crystal Formations",
    "crystal": "Crystals/Crystal Formations", "gemstones": "Crystals/Crystal Formations",
    "cash": "Cash/Dollar Bills/Money", "dollar bills": "Cash/Dollar Bills/Money",
    "money": "Cash/Dollar Bills/Money", "dollar": "Cash/Dollar Bills/Money",
    "banknotes": "Cash/Dollar Bills/Money", "bills": "Cash/Dollar Bills/Money",
    "surfboard": "Surfboard", "surf board": "Surfboard", "surfboards": "Surfboard",
...
def post_process(result, name="", symbol_names=None, game_corrections=None, game_description=""):
    ...
    # Fix 3: Normalize colors (25% area-threshold based)
    COLOR_AREA_THRESHOLD = 25
    raw_pct = result.get('art_color_tone_raw', [])
    has_raw_pct = (raw_pct and isinstance(raw_pct, list)
                   and any(isinstance(e, dict) for e in raw_pct))
    if has_raw_pct:
        filtered = []
        filtered_pct = []
        seen_colors = set()
        for entry in raw_pct:
            ...
            if nc and nc not in seen_colors and pct >= COLOR_AREA_THRESHOLD:
                filtered.append(nc)
                filtered_pct.append({'color': nc, 'pct': round(pct)})
                seen_colors.add(nc)
        if not filtered and raw_pct:
            for fallback_entry in raw_pct:
                if not isinstance(fallback_entry, dict):
                    continue
                top_color = normalize_color(fallback_entry.get('color', ''))
                if top_color:
                    fb_pct = fallback_entry.get('pct', 0)
                    if not isinstance(fb_pct, (int, float)):
                        try:
                            fb_pct = float(str(fb_pct).replace('%', '').strip())
                        except (ValueError, TypeError):
                            fb_pct = 0
                    fb_pct = max(0, min(100, fb_pct))
                    filtered = [top_color]
                    filtered_pct = [{'color': top_color, 'pct': round(fb_pct)}]
                    fixes.append("color_threshold_fallback_top1")
                    break
        result['art_color_tone'] = filtered[:3]
        result['art_color_tone_pct'] = filtered_pct[:3]
    else:
        ...
        result['art_color_tone_pct'] = []
    result.pop('art_color_tone_raw', None)
    ...
    if game_corrections:
        ...
        # Sync art_color_tone_pct after color corrections
        if colors_remove or override_colors is not None or must_colors:
            current_pct = result.get('art_color_tone_pct', [])
            final_colors = result.get('art_color_tone', [])
            if override_colors is not None:
                result['art_color_tone_pct'] = []
            else:
                synced_pct = [e for e in current_pct
                              if isinstance(e, dict) and e.get('color') in final_colors]
                for col in final_colors:
                    if not any(e.get('color') == col for e in synced_pct):
                        synced_pct.append({'color': col, 'pct': None})
                result['art_color_tone_pct'] = synced_pct[:3]
...
def classify_game(client, system_prompt, fname, use_vision=True, symbol_names=None,
    ...
    if (not review or len(review) < 50) and game_description and len(game_description) >= 50:
        review = game_description
    ...
    if (not review or len(review) < 50) and not screenshot_b64:
        return None, name, 'no review and no screenshot', {}
    ...
    _debug_color_raw = result.get('art_color_tone_raw', [])
    result, fixes = post_process(result, name, symbol_names, game_corrections, game_description)
    result['_has_screenshot'] = has_image
    result['_debug_color_raw'] = _debug_color_raw

    # Element verification pass disabled for batch runs — didn't improve GT accuracy
    # and doubles API cost. Keep code for future experimentation.
    # if masked_b64 and result.get('screenshot_quality') == 'gameplay':
    #     extra_elems = _element_verification_pass(...)
    pass

    return result, name, fixes, usage_data


def _element_verification_pass(client, masked_b64, existing_elements, bg_description):
    """Focused second pass on MASKED image (reel grid blacked out) to catch missed elements.
    Uses masked image to prevent confusing reel symbols with background elements."""
    already = ', '.join(existing_elements) if existing_elements else 'nothing'
    ELEM_SCAN_PROMPT = (
        f"The reel grid area has been blacked out. Everything you see IS background/decoration.\n\n"
        f"Already identified: [{already}]\n"
        f"Background description: \"{bg_description}\"\n\n"
        f"Look carefully at ALL visible areas. What distinct objects or features are visible "
        f"that were NOT already listed? Scan:\n"
        f"- Left/right side panels\n"
        f"- Top area above the black rectangle\n"
        f"- Bottom area below it\n"
        f"- Corners and edges\n\n"
        f"Return ONLY a JSON array of missed items. Be specific (e.g. \"surfboard\", \"crystals\", "
        f"\"dollar bills\", \"tiki mask\"). If nothing was missed, return []"
    )
    try:
        resp = client.messages.create(
            model=MODEL,
            ...
            messages=[{"role": "user", "content": [
                {"type": "image", "source": {"type": "base64", "media_type": "image/jpeg", "data": masked_b64}},
                {"type": "text", "text": ELEM_SCAN_PROMPT},
            ]}],
        )
        raw = resp.content[0].text.strip()
        raw = re.sub(r'^```\w*\n?', '', raw)
        ...
        for item in items:
            ne = normalize_element(item)
            if ne and ne not in existing_set:
...
def load_non_gameplay_set():
    """Load set of filenames known to be non-gameplay (promotional/splash/poster).
    Sources: existing results.json + prescreen_results.json."""
    non_gameplay = set()
    if os.path.exists(RESULTS_PATH):
        try:
            with open(RESULTS_PATH) as f:
                data = json.load(f)
            for fname, g in data.get('games', {}).items():
                if g.get('screenshot_quality') in ('promotional', 'rules_page'):
                    non_gameplay.add(fname)
                if g.get('art_status') == 'needs_gameplay_screenshot':
                    non_gameplay.add(fname)
        except (json.JSONDecodeError, KeyError):
            pass
    if os.path.exists(PRESCREEN_RESULTS_PATH):
        try:
            with open(PRESCREEN_RESULTS_PATH) as f:
                ps = json.load(f)
            for fname, verdict in ps.items():
                if isinstance(verdict, dict):
                    if verdict.get('classification') == 'not_gameplay':
                        non_gameplay.add(fname if fname.endswith('.html') else fname + '.html')
                elif verdict == 'not_gameplay':
                    non_gameplay.add(fname if fname.endswith('.html') else fname + '.html')
        except (json.JSONDecodeError, KeyError):
            pass
    return non_gameplay
...
def run_batch(files, use_vision=True, output_path=None, use_masked=True, use_cache=True):
    ...
    non_gameplay = load_non_gameplay_set()
    ...
    for i, fname in enumerate(files):
        if fname in non_gameplay:
            skipped_non_gameplay += 1
            print(f"\n[{i + 1}/{len(files)}] {fname}  SKIP (known non-gameplay)", flush=True)
            continue
        ...
        try:
            ...
            entry = {
                ...
                'art_color_tone_pct': result.get('art_color_tone_pct', []),
                ...
                '_debug_color_raw': result.get('_debug_color_raw', []),
            }
            ...
            _raw_pct_display = entry.get('_debug_color_raw', [])
            if _raw_pct_display:
                pct_str = ', '.join(f"{e.get('color','')}:{e.get('pct',0)}%" for e in _raw_pct_display if isinstance(e, dict))
                print(f"  Colors:   {entry['art_color_tone']}  (raw: {pct_str})", flush=True)
            else:
    ...
    if skipped_non_gameplay:
        print(f"\n  Skipped {skipped_non_gameplay} known non-gameplay screenshots (posters/splash)", flush=True)
...
def run_batch_api(files, use_vision=True):
    ...
    for fname in files:
        ...
        name_preview = fname.replace('.html', '').replace('-', ' ')
        game_desc = find_description_for_game(desc_index, name_preview)
        if (not review or len(review) < 50) and game_desc and len(game_desc) >= 50:
            review = game_desc
        ...
        if (not review or len(review) < 50) and not screenshot_b64:
            print(f"  SKIP {fname}: no review and no screenshot", flush=True)
            continue
        ...
        custom_id = re.sub(r'[^a-zA-Z0-9_-]', '', fname.replace('.html', '').replace('.', '_'))[:64]
        requests.append({
            "custom_id": custom_id,
            "params": {
                "model": MODEL,
    ...
    id_to_fname = {re.sub(r'[^a-zA-Z0-9_-]', '', f.replace('.html', '').replace('.', '_'))[:64]: f for f in valid_files}
        ...
        if result.result.type == "succeeded":
            try:
                ...
                entry = {
                    ...
                    'art_color_tone_pct': processed.get('art_color_tone_pct', []),
...
GATE_THRESHOLDS = {
    ...
    'art_characters': 85.0,
    'art_elements': 60.0,
    'art_color_tone': 70.0,  # Jaccard-based (was 88% subset); recalibrate after GT update
}
...
def run_regression():
    ...
    for r in results:
        ...
        for d in dims:
            ...
            if d == 'art_color_tone':
                ...
                r_set.discard(None)
                color_total += 1
                top1_ok = gt_list[0] in r_set if gt_list else True
                if top1_ok:
                    color_top1_matches += 1
                union = gt_set | r_set
                intersection = gt_set & r_set
                jaccard = len(intersection) / len(union) if union else 1.0
                if jaccard >= 0.5:
                    matches += 1
                    dim_scores[d]['match'] += 1
            elif d in ('art_characters', 'art_elements'):
    ...
    for d in dims:
        ...
        extra = ""
        if d == 'art_color_tone' and color_total > 0:
            top1_pct = color_top1_matches / color_total * 100
            extra = f"  (Top-1: {color_top1_matches}/{color_total} = {top1_pct:.0f}%)"
        print(f"  {d}: {s['match']}/{s['total']} = {pct:.1f}%{extra}")
    if color_total > 0:
        print(f"\n  Color scoring: Jaccard >= 0.5 = pass (Top-1 match reported separately)")
...
def repair_screenshots():
    ...
    if repaired > 0:
        ...
        for rf in repair_files:
            print(f"  {rf}")
        print(f"\nRun: python3 classify_art.py --batch-api {' '.join(repair_files)}")
...
def main():
    ...
    if args.reclassify_old:
        ...
        if old_games:
            ...
            for g in sorted(old_games):
                print(f"  {g}")
            print(f"\nTo reclassify all: python3 classify_art.py --no-masked {' '.join(sorted(old_games)[:5])} ...")
        return

    if not args.files:
        print("Usage:")
        print("  python3 classify_art.py file1.html file2.html  # classify specific games")
        print("  python3 classify_art.py --select-batch 50       # pick 50 unreviewed games")
        print("  python3 classify_art.py --regression            # run GT regression test")
        print("  python3 classify_art.py --stats                 # show pipeline stats")
        sys.exit(1)
    ...
    if args.gt_compare and os.path.exists(GT_V2_PATH):
        ...
        for r in results:
            ...
            # Colors — Jaccard >= 0.5 + Top-1 match
            gt_color_list = gt.get('art_color_tone', [])
            gt_colors = set(gt_color_list)
            r_colors = set(r.get('art_color_tone', []))
            union = gt_colors | r_colors
            intersection = gt_colors & r_colors
            jaccard = len(intersection) / len(union) if union else 1.0
            top1_ok = gt_color_list[0] in r_colors if gt_color_list else True
            if jaccard >= 0.5 and top1_ok:
                matches['color'] += 1
                print(f"  Colors: ✓ {list(r_colors)} (J={jaccard:.2f}, top1={gt_color_list[0] if gt_color_list else '-'})")
            else:
                print(f"  Colors: ✗ got={list(r_colors)}, expected={list(gt_colors)}, J={jaccard:.2f}, top1_ok={top1_ok}")
