...
| File | Purpose |
|------|---------|
| `enrich_websearch.py` | **The pipeline.** Everything in one file: extraction, normalization, post-processing, batch support. |
...
| `games_dashboard.json` | **Enrichment output.** Features, themes, symbols, descriptions, demo URLs for 642 games. |
| `games_dashboard_meta.json` | Per-game enrichment metadata (completeness, model used, batch ID, timestamp). |
| `ground_truth_ags.json` | Ground truth for accuracy measurement (173 entries, 137 evaluable). |
| `ags_vocabulary.json` | Canonical vocabulary: 26 themes, 11 features. |
| `synonym_mapping.json` | Post-LLM normalization aliases. |
