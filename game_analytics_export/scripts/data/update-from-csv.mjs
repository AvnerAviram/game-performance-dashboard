...
function runPhase1(args) {
    ...
    const masterPath = join(DATA_DIR, 'game_data_master.json');
    ...
    if (args.apply) {
        ...
        atomicWriteJson(join(DATA_DIR, 'game_data_master.json'), nextMaster);
    }
...
function runPhase2(args, baseMasters) {
    ...
    atomicWriteJson(join(DATA_DIR, 'game_data_master.json'), master);
