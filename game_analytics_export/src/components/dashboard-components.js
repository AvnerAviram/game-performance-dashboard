...
export const GameListItem = (game, index) => {
    const provider = F.provider(game);
    const theoWin = F.theoWin(game);
    const gameName = game.name || game.game_name || 'Unknown Game';
    const theoVal = typeof theoWin === 'number' ? theoWin.toFixed(1) : theoWin;
    const rankBadge =
        typeof index === 'number'
            ? `<span class="flex-shrink-0 w-5 h-5 rounded-full ${index < 3 ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'} flex items-center justify-center text-[10px] font-bold">${index + 1}</span>`
            : '';
...
export const VolatilityBadge = volatility => {
    const normalized = normalizeVolatility(volatility);
    const cls = VOL_BADGE_CLASSES[normalized] || VOL_BADGE_CLASSES['Medium'];
