...
export function predictGameSuccess() {
    ...
    const avgMechSmartIndex = mechData.length ? mechData.reduce((sum, m) => sum + (m['Smart Index'] || 0), 0) / mechData.length : 0;
    ...
    if (totalScore >= 75) { categoryClass = 'category-excellent'; categoryLabel = '✨ Excellent Potential'; }
    else if (totalScore >= 50) { categoryClass = 'category-good'; categoryLabel = '👍 Good Concept'; }
    else if (totalScore >= 25) { categoryClass = 'category-average'; categoryLabel = '⚠️ Needs Work'; }
    else { categoryClass = 'category-poor'; categoryLabel = '❌ High Risk'; }
...
function analyzeGameConcept() {
    ...
    const shortF = {'Cash On Reels':'Cash On Reels','Expanding Reels':'Expanding Reels','Free Spins':'Free Spins','Hold and Spin':'Hold & Spin','Nudges':'Nudges','Persistence':'Persistence','Pick Bonus':'Pick Bonus','Respin':'Respin','Static Jackpot':'Jackpot','Wheel':'Wheel','Wild Reels':'Wild Reels'};
    ...
    allThemes.forEach(t => { if (lower.includes(t.toLowerCase()) && t.length > 3) detectedThemes.add(t); });
    ...
    FEATS.forEach(f => { if (lower.includes(f.toLowerCase())) detectedFeats.add(f); });
            ...
            <div class="p-4"><div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ${showGames.map(g => {
                    ...
                    return `<div class="flex items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50 hover:bg-indigo-50 dark:hover:bg-indigo-900/10 transition-colors cursor-pointer" onclick="${safeOnclick('window.showGameDetails', g.name)}">
                        <div class="flex-1 min-w-0"><div class="text-sm font-semibold text-gray-900 dark:text-white truncate">${escapeHtml(g.name)}</div><div class="text-[10px] text-gray-400">${escapeHtml(g.provider_studio || '')} · ${gFeats.slice(0,2).join(', ')}${gFeats.length > 2 ? ' +' + (gFeats.length-2) : ''}</div></div>
    ...
    if (primaryTheme && themeData.length > 0) {
        const pThemeGames = allGames.filter(g => (g.theme_consolidated || '').toLowerCase() === primaryTheme.toLowerCase());
        const themeAvg = pThemeGames.length > 0 ? pThemeGames.reduce((s, g) => s + (g.performance_theo_win || 0), 0) / pThemeGames.length : 0;
    ...
    if (primaryTheme && feats.length < 3) {
        const pThemeGames = allGames.filter(g => (g.theme_consolidated || '').toLowerCase() === primaryTheme.toLowerCase());
        const themeAvg = pThemeGames.length > 0 ? pThemeGames.reduce((s, g) => s + (g.performance_theo_win || 0), 0) / pThemeGames.length : 0;
        let bestUnselected = null, bestLift = -Infinity;
        FEATS.forEach(f => {
