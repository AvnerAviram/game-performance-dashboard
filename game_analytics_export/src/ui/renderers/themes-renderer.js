...
function updateThemesPaginationInfo(total, start, end) {
    const themesPerPage = window.themesPerPage ?? 50;
    ...
    themes.forEach((theme, index) => {
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-3.5">
                ${isUnified ? `<span class="unified-theme-name cursor-pointer font-semibold text-gray-900 dark:text-white" onclick="toggleSubThemes(${globalIndex})">${expandIcon}${escapeHtml(themeName)}</span>` : `<span class="theme-link cursor-pointer font-semibold text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors" data-theme="${themeNameEscaped}">${escapeHtml(themeName)}</span>`}
            </td>
